...
# =====================
# Phase 3: INT8 service for Qwen2-1.5B (Windows + CUDA + bitsandbytes)
...
# Concurrency: 1 on a 6GB VRAM card is the safe default
CONCURRENCY_LIMIT = 1
sem = asyncio.Semaphore(CONCURRENCY_LIMIT)

app = FastAPI(title="Qwen2-1.5B Local Service (INT8)")
...
def _encode_tail(text: str, ctx_len: int):
    """
    Keep last ctx_len tokens (tail truncation) to preserve the newest user question.
    This avoids the "question gets truncated away" issue on long prompts.
    """
...
async def _acquire_or_429():
    """
    Fast reject if semaphore is not immediately available.
    This is more stable on Windows + long GPU inference than queueing many requests.
    """
    # NOTE: asyncio.Semaphore has no non-blocking acquire, so we peek _value.
    # This is acceptable for our single-process local service experiment.
...
def _startup():
    ...
    # INT8 quantization config (BitsAndBytes)
    bnb_config = BitsAndBytesConfig(load_in_8bit=True)

    # IMPORTANT:
    # - Do NOT pass torch_dtype here; INT8 uses quantization_config
    # - device_map="cuda" keeps it on GPU
...
async def generate_stream(req: GenerateReq, request: Request):
    ...
    # fast reject when busy (no queueing for streaming)
    await _acquire_or_429()

    queue_wait_s = 0.0  # schema completeness (fast reject design)
    ...
    def _run_generate():
        try:
            ...
            # streamer will stop naturally when generation fails
            pass
